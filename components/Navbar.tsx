...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      setActive(window.location.hash || "#projects");
    };
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-slate-950/60 backdrop-blur-md border-b border-white/10 flex justify-between items-center px-6 md:px-12 py-4">
      ...
      <div className="hidden md:flex items-center gap-8">
        <a href="#projects" className={getClass("#projects")}>
          Projects
        </a>

        <a href="#tech" className={getClass("#tech")}>
          Tech
        </a>

        <a href="#skills" className={getClass("#skills")}>
          Skills
        </a>

        <a href="#contact" className={getClass("#contact")}>
          Contact
        </a>
      </div>
      ...
      <a
        ...
        rel="noopener noreferrer"
      >
        <button className="bg-slate-950 border border-primary-container text-primary-container px-6 py-2 font-label-mono text-sm uppercase hover:bg-primary-container hover:text-on-primary-container transition-all flex gap-2 items-center">
          Get in Touch
          <FaWhatsapp />
        </button>
      </a>
